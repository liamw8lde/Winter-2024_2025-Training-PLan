...
# ---------- Styles (fix select/multiselect for dark) ----------
CUSTOM_CSS = '''<style>
:root {
...
/* ---- Streamlit select/multiselect dark patch ---- */
.stMultiSelect, .stSelectbox { --border: 1px solid var(--divider); }
...
div[data-baseweb="select"] > div:hover { border: 1px solid #2f3a46 !important; }
div[data-baseweb="select"] > div:focus-within { border: 1px solid var(--badge-edge) !important; }
...
div[role="option"][aria-selected="true"] {
  background: #0e2215 !important; color: var(--accent) !important;
}

/* kill accidental error/red state */
div[data-baseweb="select"] > div[aria-invalid="true"] { border: 1px solid var(--divider) !important; }
</style>'''
...
def render_week_view(df, year, week):
    ...
    st.markdown(
        '<div class="page-title"><h1>Wochenplan'
        ' <span class="source-pill">• Quelle: GitHub (read-only)</span></h1></div>',
...
# ---------- Load data (read-only from GitHub) ----------
try:
...
with tab1:
    ...
    if not week_keys:
        ...
        if "wk_idx" not in st.session_state:
            # Start on the latest week by default:
            st.session_state.wk_idx = len(week_keys) - 1
