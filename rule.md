...
- Trim spaces, collapse multiple spaces, handle special characters (e.g., ß ↔ ss, umlauts), and normalize case.
...
- **Wednesday (Mittwoch)**:
  - `E18:00-60 PLA` — Einzel (PLB at 18:00 is illegal)
...
- **Dirk Kistner** → Mo/Mi/Do only; on Wednesday only 19:00; max 2/week.
- **Arndt Stueber** → only Wednesday 19:00.
- **Jens Hafner** → only Wednesday 19:00.
- **Thomas/Thommy Grueneberg** → start 18:00 or 19:00 70% of the time; 30% Wednesday 20:00.
...
3. **Holiday Exception**:
- If >1 core player is missing and not on holiday → **violation** `mon_pla_core` (do NOT mark as UNREPAIRABLE during the first loop).
4. **Enforcement Order**:
...
- **Singles Rank Window (HARD)**:
  - For players A, B in singles: `|rank[A] − rank[B]| ≤ 2` → else violation `singles_rank_window`.
- **Doubles Balance (SOFT/advisory)**:
  - With sorted ranks `r1 ≤ r2 ≤ r3 ≤ r4`, flag `doubles_unbalanced_advisory` if neither:
...
- Do not delete rows. If unsolvable without breaking a HARD rule:
  ...
  - Only after exhausting all solutions, append “(UNREPAIRABLE)” to `Spieler` and keep the row.
- Maintain headcount and slot legality at all times.

### REPAIR PRIORITY ORDER
1. Illegal slot codes / wrong weekday-duration-court → fix to nearest legal slot or re-audit.
...
4. Holidays/blackouts (except Monday core exemption) → replace with legal, available players.
5. Protected player time rules → move/replace to legal times.
6. One-per-day conflicts → de-duplicate by swapping with legal alternates.
7. Weekly coverage (missing/duplicate required slots) → fix within week where possible.
8. Weekly & season caps → re-balance without breaking earlier HARD rules.
9. Singles rank window (HARD) → re-pair; if impossible, re-audit.
10. Doubles balance (SOFT) → improve without breaking HARD rules.
...
- Append “(UNREPAIRABLE)” to unresolved rows.
