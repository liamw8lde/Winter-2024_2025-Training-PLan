...
# ==================== PAGE CONFIG ====================
st.set_page_config(
    page_title="Training Plan Auto-Population",
    layout="wide",
    initial_sidebar_state="collapsed"
)

st.title("🎾 Training Plan Auto-Population")
st.caption("Automatically fill empty training slots with load-balanced player assignments")
...
def load_plan_csv(file_path="Winterplan.csv"):
...
@st.cache_data(show_spinner=False)
def load_preferences_csv(file_path="Spieler_Preferences_2026.csv"):
...
# ==================== SLOT GENERATION ====================
def generate_allowed_slots_calendar(df):
    """Generate all allowed slots for the season"""
    start_date = pd.to_datetime(df["Datum_dt"]).dropna().min().date()
    end_date = pd.to_datetime(df["Datum_dt"]).dropna().max().date()
    ...
    out = []
    current = (int(start_iso.year), int(start_iso.week))
    end = (int(end_iso.year), int(end_iso.week))

    while True:
        y, w = current
        for tag, slot_list in ALLOWED_SLOTS.items():
            iso_wd = WEEKDAY_TO_ISO[tag]
            dt = pd.Timestamp.fromisocalendar(y, w, iso_wd).date()
...
def find_empty_slots(df_plan):
    ...
    allowed = generate_allowed_slots_calendar(df_plan)
...
def github_put_file(csv_bytes, message):
    """Save CSV to GitHub"""
...
with st.spinner("Lade Daten..."):
    df_plan, df_exp = load_plan_csv()
    df_prefs = load_preferences_csv()

if df_plan is None:
    st.error("Konnte Trainingsplan nicht laden!")
    st.stop()
...
with col2:
    st.metric("Spieler in Präferenzen", len(all_players))
with col3:
...
if empty_slots:
    st.header("📋 Leere Slots gefunden")
    st.write(f"Es gibt **{len(empty_slots)}** leere Slots, die gefüllt werden können.")

    # Show sample
    with st.expander(f"Zeige erste {min(10, len(empty_slots))} leere Slots"):
        for slot in empty_slots[:10]:
    ...
    if "df_result" in st.session_state:
        ...
        with col_save:
            if st.button("💾 Auf GitHub speichern", use_container_width=True, type="primary"):
                try:
                    df_to_save = st.session_state.df_result[["Datum", "Tag", "Slot", "Typ", "Spieler"]]
                    csv_bytes = df_to_save.to_csv(index=False).encode("utf-8")
                    msg = f"Auto-populate {len(filled)} slots with load balancing\n\n🤖 Generated with Claude Code"
                    github_put_file(csv_bytes, msg)
        ...
        with col_download:
            ...
            st.download_button(
                "📥 CSV herunterladen",
                data=csv_bytes,
                file_name="Winterplan_autopopulated.csv",
                mime="text/csv",
    ...
    st.success("🎉 Keine leeren Slots gefunden! Der Plan ist vollständig.")

# Documentation
st.markdown("---")
with st.expander("ℹ️ Wie funktioniert die Auto-Population?"):
    st.markdown("""
    ### Auto-Population Algorithmus

    **1. Leere Slots identifizieren**
    - Generiert alle erlaubten Slots basierend auf Wochentagen und Zeitfenstern
    - Vergleicht mit aktuellem Plan, um leere Slots zu finden

    **2. Spieler-Kandidaten bewerten**
    - Verfügbarkeit: Prüft Wochentag-Verfügbarkeit aus CSV
    - Urlaube: Prüft BlockedRanges und BlockedDays
    - Präferenzen: Berücksichtigt "nur Einzel" / "nur Doppel"
    - Geschützte Regeln: Spezielle Zeitfenster für bestimmte Spieler
    - Nutzung: Zählt bisherige Einsätze (Woche & Saison)

    **3. Load Balancing**
    - Priorisiert Spieler mit weniger Saison-Einsätzen
    - Sekundär: Weniger Wochen-Einsätze
    - Tertiär: Spieler-Ranking

    **4. Regelkonformität**
    - **Einzel:** Rang-Unterschied zwischen Spielern ≤ 2
    - **Doppel:** Genau 4 verfügbare, legale Spieler
    - Frauen-Einzel-Verbot wird beachtet

    **5. Zuweisung**
    - Füllt Slots mit am wenigsten genutzten, legalen Spielern
    - Überspringt Slots, wenn keine geeigneten Spieler verfügbar

    ### Datenquellen
    - **Winterplan.csv:** Aktueller Trainingsplan
    - **Spieler_Preferences_2026.csv:** Verfügbarkeit, Präferenzen, Urlaube
    - **RANK:** Hart-codierte Spieler-Rankings (1=stärkster, 6=schwächster)
    - **Geschützte Regeln:** Spezielle Einschränkungen für bestimmte Spieler

    ### Tipps
    - Starten Sie mit einer kleinen Anzahl Slots (10-20) für erste Tests
    - "Nur legale Zuweisungen" stellt sicher, dass keine Regelverstöße entstehen
    - Nutzen Sie die Vorschau, bevor Sie speichern
    - Prüfen Sie die Spieler-Verteilung für Fairness
    """)
